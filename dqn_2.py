...
class QNet(nn.Module):
    ...
    def forward(self, obs, action):
        ...
        if random.random() < 0.2: # 20% random action
            return torch.tensor(random.randint(0, self.action_dim-1), dtype=torch.int64)
        else:
            with torch.no_grad():
    ...
    def train(self, dataset, sample_size=50):
        ...
        with torch.no_grad():
            ...
            action = torch.tensor([item[1] for item in minibatch], dtype=torch.float32)
            ...
            terminal_state = torch.tensor([item[4] for item in minibatch], dtype=torch.float32)
...
if __name__ == "__main__":
    ...
    for episode in range(10000):
        ...
        for _ in range(200):
            ...
            action_idx = model.forward(torch.tensor(obs, dtype=torch.float32).view(1, -1)) # model(obs)
            ...
            dataset.append((obs, model.all_actions[action_idx].numpy(), new_obs, reward, int(terminated)))
            obs = new_obs
